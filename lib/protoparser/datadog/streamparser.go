	...
	for i := range series {
		...
		if *sanitizeMetricName {
			...
			case traceutil.ErrEmpty:
				logger.Warnf("Fixing malformed trace. Name is empty (reason:span_name_empty), setting span.name=%s: %s", originalValue, series[i].Metric)
			case traceutil.ErrTooLong:
				logger.Warnf("Fixing malformed trace. Name is too long (reason:span_name_truncate), truncating span.name to length=%d: %s", traceutil.MaxServiceLen, series[i].Metric)
			case traceutil.ErrInvalid:
				logger.Warnf("Fixing malformed trace. Name is invalid (reason:span_name_invalid), setting span.name=%s: %s", originalValue, series[i].Metric)
			}
		}
	}
	rowsRead.Add(rows)
